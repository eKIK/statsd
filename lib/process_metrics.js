...
var process_metrics = function (config, metrics, flushInterval, ts, flushCallback) {
    ...
    for (key in timers) {
      ...
      if (timers[key].length > 0) {
        ...
        for (key2 in pctThreshold) {
          ...
          if (metricsConfig.count_pct !== false) current_timer_data["count_" + clean_pct] = numInThreshold;
          if (metricsConfig.mean_pct !== false) current_timer_data["mean_" + clean_pct] = mean;
          if (metricsConfig.sum_pct !== false) current_timer_data["sum_" + clean_pct] = sum;
          if (metricsConfig.sum_squares_pct !== false) current_timer_data["sum_squares_" + clean_pct] = sumSquares;
          if (pct > 0) {
            if (metricsConfig.upper_pct !== false) current_timer_data["upper_" + clean_pct] = thresholdBoundary;
          } else {
            if (metricsConfig.lower_pct !== false) current_timer_data["upper_" + clean_pct] = thresholdBoundary;
          }
        ...
        if (metricsConfig.std !== false) current_timer_data["std"] = stddev;
        if (metricsConfig.upper !== false) current_timer_data["upper"] = max;
        if (metricsConfig.lower !== false) current_timer_data["lower"] = min;
        if (metricsConfig.count !== false) current_timer_data["count"] = timer_counters[key];
        if (metricsConfig.count_ps !== false) current_timer_data["count_ps"] = timer_counters[key] / (flushInterval / 1000);
        if (metricsConfig.sum !== false) current_timer_data["sum"] = sum;
        if (metricsConfig.sum_squares !== false) current_timer_data["sum_squares"] = sumSquares;
        if (metricsConfig.mean !== false) current_timer_data["mean"] = mean;
        if (metricsConfig.median !== false) current_timer_data["median"] = median;
        ...
        if(bins.length) {
            if (metricsConfig.histogram !== false) current_timer_data['histogram'] = {};
        }
        ...
        for (var bin_i = 0; bin_i < bins.length; bin_i++) {
          ...
          if (metricsConfig.histogram !== false) current_timer_data['histogram'][bin_name] = freq;
        }

      } else {
        if (metricsConfig.count !== false) current_timer_data["count"] = 0;
        if (metricsConfig.count_ps !== false) current_timer_data["count_ps"] = 0;
      }
